...
void VBOMesh::loadOBJ( const char * fileName, bool reCenterMesh ) {
        ...
        if( line.length( ) > 0 && line.at(0) != '#' ) {
            ...
            if (token == "v" ) {
				...
				else
				{
					...
					if( faceTex.size() == 3 )
					{						
						if(faceTex[0]<points.size())
						{
							_texCoords[face[0]]=texCoords[faceTex[0]];					
						}										
						if(faceTex[1]<points.size())
						{
							_texCoords[face[1]]=texCoords[faceTex[1]];		
						}
						if(faceTex[2]<points.size())
						{
							_texCoords[face[2]]=texCoords[faceTex[2]];		
						}						
					}
                }
            }
    ...
    if( texCoords.size() > 0 )
	{
		...
		{
			generateTangents(points,normals,faces,_texCoords,tangents);
		}
		else
		{
	...
	if(_texCoords.size() > 0)
	{
		storeVBO(points, normals, _texCoords, tangents, faces);
	}
	else
	{
	...
	if(_texCoords.size() > 0)
	{
		cout << " " << _texCoords.size() << " texture coordinates." << endl;
	}
	else
	{
